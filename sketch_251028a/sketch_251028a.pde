...
void draw(){
  ...
  timerForZombie++;
  zno1.showZombie();
  circle(zno1.zX,zno1.hitPointY,5);   // this hit point somehow works fine. search how. why the last method not   // 
  gunPng.showGun();
  ...
  ball.pointerMove();
  popStyle();
    if((dist(zno1.zX, zno1.hitPointY, ball.pointerX, ball.pointerY) <= 50) && key == 'b'){       // becarefull if the "caps lock" on then the key b is not working
      // bellow here only checking the value
      println(zno1.hitPointX);
